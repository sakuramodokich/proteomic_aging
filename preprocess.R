...
cov <- data.table::fread("/mnt/vol2/UKB_dataset/asset/UKB_merged_dataset_23_10_recoded.tsv", select=c('eid', map_ids)) %>% as_tibble
cov$sex <- cov$p31
cov$age <- cov$p21022
...
# GWAS
dat <- data.table::fread("main_aging_dataset_new.csv") %>% as_tibble
req <- c("p31","p22001","p22006","p22019","p22021","p22027","p21022","p22000",paste0("p22009_a",1:20))
cov <- data.table::fread("/mnt/vol2/UKB_dataset/asset/UKB_merged_dataset_23_10_recoded.tsv", select=c('eid', req)) %>% as_tibble
cov$age <- cov$p21022
...
keep <- tibble(FID=cov$eid, IID=cov$eid)
keep %>% write_delim("LD_keep_sample.txt")
...
pro <- data.table::fread("~/vol2/UKB_dataset/asset/olink_protein_wide2923_QC_knn_impute_INT.tsv") %>% pull(eid)
