...
icd_epistart <- data.table::fread("/mnt/vol2/UKB_dataset/phewas_prep/icd_epistart.tsv", header = T) %>% semi_join(dat, "eid") %>% keep(~sum(!is.na(.)) >= 200)
annot <- read_csv("/mnt/vol2/UKB_dataset/phewas_prep/Phecode_map_v1_2_icd10cm_beta.csv", col_types = list("phecode"="character")) %>% filter(!duplicated(phecode))

multicox.tidy <- possibly(function(data, trait, exposure, covariates){
  icd <- data.table::fread(paste0('/mnt/vol2/UKB_dataset/phewas_prep/scatter/icd_epistart_',trait,".tsv"), header = T) %>% dplyr::rename("y"=trait) %>% semi_join(data, "eid")
  ...
  res <- coxph(as.formula(formula), data=data) %>% broom::tidy(exp=T, conf.int=T)
  res$term <- trait
  res$N <- nrow(data)
