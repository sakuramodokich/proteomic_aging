...
dict <- data.table::fread("data_dictionary.csv") %>% filter(entity=="participant")
...
if(F){ # preprocess self-reported data
  ...
  dd %>% write_csv("non_cancer_self_reported.csv")
}
self_non_ca <- data.table::fread("non_cancer_self_reported.csv") %>% as_tibble
...
# Healthspan
icd10 <- data.table::fread("inpatient_diagnosis.csv")

death <- data.table::fread("death.csv") %>% arrange(eid)
...
dat %>% write_csv("main_aging_dataset.csv")
